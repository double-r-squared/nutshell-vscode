...
function listFiles(absRoot, log) {
  ...
  // server-side.
  //
  // Read failures are non-fatal but logged. Common culprits: macOS TCC
  // (Documents / Desktop / iCloud Drive blocked until VS Code is granted
  // access in Privacy settings), restrictive directory permissions, or
  // a path that vanished between the scan starting and reading a sub-
  // directory. Without surfacing them the symptom was "project
  // registers with 0 files" — looking like the path was empty.
  const out = []
  const errors = []
  const walk = (dir) => {
    let entries
    try {
      ...
      errors.push({ dir, code: err.code, message: err.message })
      return
    }
  ...
  walk(absRoot)
  if (errors.length && typeof log === 'function') {
    const firstFew = errors
      .slice(0, 3)
      .map((e) => `${e.dir} (${e.code || 'error'})`)
      .join(', ')
    const more = errors.length > 3 ? ` +${errors.length - 3} more` : ''
    log(`[push] ${errors.length} read error${errors.length !== 1 ? 's' : ''} under ${absRoot}: ${firstFew}${more}`)
  }
  return out
}
...
function scanAll(roots, log) {
  const files = []
  for (const { sourceRoot, prefix } of roots) {
    for (const abs of listFiles(sourceRoot, log)) {
      try {
        ...
        if (typeof log === 'function') {
          log(`[push] read failed for ${abs}: ${err.code || 'error'} ${err.message}`)
        }
      }
    }
  }
