...
// empty.
// Resolve docSources to {linkName, absPath} entries with collision handling.
// Used by both the symlink builder (fs mode) and the push driver (remote
// mode), so namespace IDs (`<linkName>/<relPath>`) match across modes.
// Skips entries whose absolute path doesn't exist.
function resolveDocSources(workspaceRoot, docSources) {
  if (!docSources || docSources.length === 0) return []
  ...
  for (const relPath of docSources) {
    ...
    desired.set(linkName, absPath)
  }
  return [...desired.entries()].map(([linkName, absPath]) => ({ linkName, absPath }))
}
...
function rebuildVirtualRoot(workspaceRoot, docSources) {
  ...
  const desired = new Map()
  for (const { linkName, absPath } of resolveDocSources(workspaceRoot, docSources)) {
    desired.set(linkName, absPath)
  }
